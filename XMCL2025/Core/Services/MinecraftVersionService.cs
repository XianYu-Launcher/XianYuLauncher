    ...
    private async Task DownloadNeoForgeVersionAsync(string minecraftVersionId, string neoforgeVersion, string versionsDirectory, string librariesDirectory, Action<double> progressCallback, CancellationToken cancellationToken = default, string customVersionName = null)
    {
        try
        {
            ...
            using (var response = await _httpClient.GetAsync(clientJarUrl, HttpCompletionOption.ResponseHeadersRead))
            {
                ...
                using (var stream = await response.Content.ReadAsStreamAsync())
                using (var fileStream = new FileStream(neoforgeJarPath, FileMode.Create, FileAccess.Write, FileShare.None, bufferSize, FileOptions.Asynchronous))
                {
                    var buffer = new byte[bufferSize];
                    int bytesRead;
                    ...
                    while ((bytesRead = await stream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (totalSize > 0)
                        {
                            double progress = 10 + ((double)totalRead / totalSize) * 10;
                            progressCallback?.Invoke(progress);
                        }
                    }
                }
            ...
            // 验证JAR文件的SHA1哈希
            progressCallback?.Invoke(20); // 20% - 开始验证JAR文件
            ...
            progressCallback?.Invoke(22); // 22% 进度用于原版文件下载
            ...
            if (extractedClientLzmaPath == null && allClientLzmaFiles.Length > 0)
            {
                extractedClientLzmaPath = allClientLzmaFiles[0];
            }
            
            // 更新进度到70%，表示完成client.lzma文件处理
            progressCallback?.Invoke(70);
            System.Diagnostics.Debug.WriteLine("[DEBUG] 已完成client.lzma文件处理，进度更新到70%");
    ...
    private async Task ProcessNeoForgeInstallProfile(string installProfilePath, string installerPath, string neoforgeVersionDirectory, string librariesDirectory, Action<double> progressCallback, string extractDirectory)
    {
        try
        {
            ...
            foreach (JObject processor in clientProcessors)
            {
                ...
                await ExecuteProcessor(processor, installerPath, neoforgeVersionDirectory, librariesDirectory, progressCallback, installProfilePath, extractDirectory, "neoforge");
